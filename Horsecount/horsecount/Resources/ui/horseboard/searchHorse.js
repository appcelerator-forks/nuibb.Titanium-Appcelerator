function HorseList(_title, windowStack, onCreate) {
	...
	function get_url(_title) {
		if (_title === 'Find by Breeder' || _title === 'Find by Owner') {
			return system_url.getBreeder_url();
		} else {
			return system_url.getBoardAllHorse_url();
		}
	}
	...
	function Fill_With_JSON(_term) {
		...
		new API_Call('POST', get_url(_title), param, activityIndicator, function(json) {
			renderJson(json);
		});
	}
